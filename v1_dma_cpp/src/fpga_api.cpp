...
FPGA::FPGA(off_t data_addr, off_t output_addr, int m_size, int v_size)
{
  ...
  ps_dram_input = static_cast<float*>(mmap(NULL, m1_size_*sizeof(float), PROT_READ|PROT_WRITE, MAP_SHARED, fd_, 0x10001000));
...
const float* __attribute__((optimize("O0"))) FPGA::blockMM(float *m1, float *m2)
{
  num_block_call_ += 1;

  memcpy(ps_dram_weight, m1, m1_size_*sizeof(float));
  memcpy(ps_dram_input, m2, m1_size_*sizeof(float));
  ...
  *(fpga_dma+10) = sizeof(float) * m1_size_;
  while ((*(fpga_dma+1) & 0x00000002) == 0);

  *(fpga_dma+6) = 0x10001000;
  *(fpga_dma+8) = 0xC0000400;
  *(fpga_dma+10) = sizeof(float) * m1_size_;
  while ((*(fpga_dma+1) & 0x00000002) == 0);
  ...
  *(fpga_dma+8) = data_addr;
  *(fpga_dma+10) = sizeof(float) * m1_size_;
  while ((*(fpga_dma+1) & 0x00000002) == 0);

  // memcpy(data_M, ps_dram_weight, sizeof(float) * m1_size_);
  clock_t end = clock();
...
void FPGA::largeMM(const float* weight_mat, const float* input_mat, float* output, int num_input, int num_output, int num_matrix2)
{
  ...
  float output_M_[num_output*num_matrix2];
  ...
  for(int i = 0; i < num_output; i += v_size_)
  {
    ...
    {			
      for(int k = 0; k < num_matrix2; k += v_size_)
      {
        ...
        const float* ret = this->blockMM(m1, m2);
        ...
        for(int n = 0; n<block_row; ++n)
        {
          ...
          {
            output_M_[(i + n) + (k + m)*num_output] += ret[n*v_size_ + m];
          }
        }
      }
    }
  ...
  memcpy(output, output_M_, sizeof(float) * num_output * num_matrix2);
}
