module pe_array_v1 (S_AXI_ACLK, S_AXI_ARESETN, start, BRAM_RDDATA, BRAM_WRDATA, BRAM_ADDR, done, BRAM_WE, BRAM_CLK);
    parameter              V_SIZE = 8;
    ...
    assign BRAM_ADDR = (BRAM_WE) ? counter_harv << 2 : counter_load << 2;
    
    always @(posedge aclk)
        if (counter_load > 0 && counter_load <= 128) begin
            if (counter_load <= 64) 
                global_buffer1[counter_load - 1] = BRAM_RDDATA;
            else
                global_buffer2[counter_load - 65] = BRAM_RDDATA;
        end
